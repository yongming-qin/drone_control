...
// darknet_ros_msgs
#include <darknet_ros_msgs/BalloonPosition.h>
#include <darknet_ros_msgs/BalloonPositions.h>
...
class YoloObjectDetector
{
 ...
 private:
   ...
   * @return the number of the filtered points
   */
  int getPositionPointcloud(const sensor_msgs::PointCloud2ConstPtr &pc, float x, float y, float w, float h);
  ...
  ros::Publisher balloonPositionsPublisher_;

  //! Detected objects.
  std::vector<std::vector<RosBox_> > rosBoxes_;
  std::vector<int> rosBoxCounter_;

  darknet_ros_msgs::BalloonPositions balloonPositionsResults_;
